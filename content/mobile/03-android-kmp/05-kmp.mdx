...
**Im Oktober 2025 hat dies leider nicht geklappt…**
...
`libs.versions.tom` erweitern um die Abhängigkeiten für Room und SQLite:
...
In `build.gradle.kts` der KMP-App die Abhängigkeiten hinzufügen (im Abschnitt `commonMain`) und auch zwei Zeilen in `plugins` 
ergänzen:
