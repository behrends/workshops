...
- In `eas.json` folgendes unter `build` hinzufügen, um eine Debug-APK zu erstellen:
  ...
  ```
- Im Projektverzeichnis die APK-Datei in der EAS-Cloud bauen: `eas build --platform android --profile preview-apk` (die Vorschläge bestätigen)
...
- Alternativ kann die APK-Datei lokal erstellt werden: `eas build --platform android --profile preview-apk --local` 
