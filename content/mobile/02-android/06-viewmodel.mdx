...
fun TodosApp() {
    ...
    // restlicher Code unverändert
            ...
            if (showDialog) {
                AddTodoDialog(
                    onAdd = { text ->
                        todos.add(Todo(text=text, done=false))
                        showDialog = false
                    },
                    onDismiss = { showDialog = false }
                )
...
@Composable
fun TodoList(todos: List<Todo>, onToggleAt: (Int) -> Unit, modifier: Modifier = Modifier) {
    LazyColumn(modifier) {
        itemsIndexed(todos) { index, todo ->
            TodoItem(todo = todo, onCheckedChange = { onToggleAt(index) })
        }
    }
}
...
MainActivity gebunden.

Zur Vorbereitung eine weitere Abhängigkeit für `lifecycle-viewmodel-compose` in 
`app/build.gradle`  hinzufügen:

```kotlin
    implementation("androidx.lifecycle:lifecycle-viewmodel-compose:2.9.4")  
```

&rarr; Wieder mit `Alt-Enter` in den „Versionskatalog“ (siehe `libs.versions.toml`)  überführen.
...
```kotlin {2-3,10,18}
@Composable
fun TodosApp(vm: TodosViewModel = viewModel()) {
    val todos = vm.todos
