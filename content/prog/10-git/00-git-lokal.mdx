...
3. In `package.json` die Zeile mit `"type": …` ändern zu `"type": "module"`.
...
`git add .` bezieht sich auf den **aktuellen Ordner** und kann Änderungen außerhalb
dieses Ordners übersehen.
...
Ohne `user.name` und `user.email` bricht `git commit` normalerweise ab
(z. B. mit „Please tell me who you are“).
...
- **E-Mail:** fiktive Mail, DHBW-Mailadresse oder GitHub `noreply`-Adresse (unter _Settings → Emails_)

Für rein lokales Arbeiten reicht auch eine fiktive Adresse (z.B. `name@localhost`). 
Vor dem Veröffentlichen auf GitHub sollte sie angepasst werden. Die Angaben lassen sich 
jederzeit mit denselben Befehlen ändern.
</Callout>
