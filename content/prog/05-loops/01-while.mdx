...
```js
while( ...ein Ausdruck, der zu true/false ausgewertet wird (truthy/falsy)... ) {
...
## `while` im Live-Coding

Wir bauen eine `while`-Schleife für das Hauptprogramm in der Wetter-App ein
und besprechen dabei verschiedene Verwendungsarten und Situationen 
(Abbruchbedingung, Endlosschleife mit Programmabbruch, usw.)

&rarr; Vielleicht genügt das bereits, denn die `while`-Schleife ist syntaktisch 
gesehen eines der einfachsten, zugleich aber auch mächtigsten Konstrukte.

<Callout>
Je nach Interesse und Aufmerksamkeit können wir die folgenden Beispiele
besprechen. Allerdings wird es im Zeitalter von KI-untertützter Programmierung
zunehmend schwieriger, die manuelle Entwicklung von Lösungen solcher abstrakten und 
„künstlich“ wirkenden Beispiele zu motivieren.

&rarr; Wir könnten direkt zu den nachfolgenden Mini-Projekten springen.
</Callout>
...
wir die restlichen Ziffern ohne die letzte Ziffer erhalten:
...
&rarr; In JavaScript erhalten wir die ganzzahlige Division
...
//    --> ermittle Ziffern ohne Ziffer ganz rechts bzw. hinten
//    tmp = tmp ohne letzte Ziffer
...
&rarr; _Computational Thinking_ („Informatisches Denken“)
...
const num = questionInt("Positive ganze Zahl eingeben:");
...
let numString = questionInt("Positive ganze Zahl eingeben:");
