...
Codequalität wichtiger: Wir brauchen klare, wiederholbare Checks.
...
Es sollten Anforderungen und Risiken geprüft werden, nicht nur die aktuelle Implementierung.

## Prettier als Fundament

Werkzeuge wie [Prettier](https://prettier.io) reduzieren unnötige Unterschiede
...
Kurzkonfiguration über `.prettierrc` (&larr; **Achtung**: `.` am Anfang des Dateinamens):
...
Für VS Code: Prettier-Extension installieren, in den Einstellungen `Format on Save` aktivieren und Prettier als Standard-Formatter auswählen (`Default Formatter`).
...
- Formatierung (z. B. mit Prettier)
- Type Checking (z. B. mit TypeScript)
- Linting (z. B. mit ESLint)
- Unit-Tests (z. B. mit Vitest)

Die folgenden Befehle werden in den nächsten Lektionen schrittweise eingerichtet:
...
- mindestens einen kritischen Ablauf als einfachen [Integrationscheck im TUI](/prog/09-quality/05-integrationscheck-tui) prüfen
