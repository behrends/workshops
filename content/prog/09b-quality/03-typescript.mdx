...
<Callout>
  **Dauer:** 30 Minuten
  ...
  - Statische Typen helfen, Fehler frühzeitig zu finden
  - Langfristige Projekte werden mit TypeScript wartbarer

  **Ziel:** TypeScript als Werkzeug zur Qualitätssicherung einsetzen
</Callout>
...
TypeScript und `tsx` (für die direkte Ausführung von
TypeScript-Dateien) werden als Entwicklungsabhängigkeiten installiert:

```bash
npm install --save-dev typescript tsx
```

Eine TypeScript-Datei (z.B. `beispiel.ts`) kann dann direkt
ausgeführt werden:
...
Fehler zur Programmierzeit, der direkt im Editor angezeigt wird.
...
In größeren Projekten wird TypeScript über eine
Datei `tsconfig.json` konfiguriert. Diese Datei
kann mit folgendem Befehl erzeugt werden:

```bash
npx tsc --init
```

Wichtige Optionen sind z.B. `strict` (strikte Typprüfung)
und `target` (Ziel-JavaScript-Version).

## Hinweis zu Bun und Deno

[Bun](https://bun.sh) und [Deno](https://deno.land/) sind
alternative JavaScript-/TypeScript-Runtimes, die TypeScript
nativ unterstützen &mdash; ohne zusätzliche Installation
von `typescript` oder `tsx`.
