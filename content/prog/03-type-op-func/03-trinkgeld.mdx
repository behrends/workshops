...
können wir `question` verwenden und den String mit `parseFloat()`
in eine Zahl umwandeln:

```js
import { question } from 'readline-sync';

let input = question("Betrag: ");
let amount = parseFloat(input);
```

Alternativ funktioniert `questionFloat` aus `readline-sync`.
</Callout>
...
--- Trinkgeld-Optionen ---
Sparsam  (10%): 8.00  → Gesamt: 88.00
Normal   (15%): 12.00 → Gesamt: 92.00
Großzügig (20%): 16.00 → Gesamt: 96.00
```
...
```js
console.log(`Sparsam (10%): ${tip10} → Gesamt: ${total10}`);
```
</Callout>
...
## Aufgabe 5: Eingabevalidierung (anspruchsvoll)
...
## Aufgabe 6: Komplettlösung (anspruchsvoll)
