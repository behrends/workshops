...
In anderen Programmiersprachen wie z. B. Java hängt das Ergebnis vom Typ der
Operanden ab (z. B. `int / int` ganzzahlig, `double / double` mit Komma). 
...
let name = question("Wie heißt Du?");
...
Typumwandlung durchgeführt, sodass hier die Zahl `4` 
mit der Zahl `4` verglichen wird. Die automatische bzw. implizite
Typumwandlung wird im Englischen _type coercion_ genannt.
...
empfohlene Herangehensweise („_best practice_“) in JavaScript.
</Callout>
...
<Callout type="info">
Wir nennen das Zeichen `&` **„Und-Zeichen“** und der **„senkrechte Strich“** `|` 
...
// fiktive Situation: Jugendliche dürfen nicht auf den Spielplatz
age >= 18 || age < 14; // true (eine ist true)
age3 >= 18 || age3 < 14; // false (beide sind false)
```

<Callout type="info">
Die einfachen Operatoren `&` und `|` sind bitweise Operatoren, welche
wir nicht behandeln. Mit diesen können bitweise Operationen durchgeführt werden,
...
Sowohl der vorangestellte Präfix-Operator (`++x`) und der 
nachfolgend notierte Postfix-Operator (`x++`) erhöhen den 
...
In der gemeinsam entwickelten Wetter-App könnten wir
abhängig von der Temperatur eine Meldung ausgeben,
z. B. „_Ziemlich kalt!_“ oder „_Nicht zu kalt_“.
