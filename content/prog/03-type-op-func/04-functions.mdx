...
später noch weitere Aspekte kennenlernen (z. B. Arrow-Funktionen,
...
const input = question("Bitte Namen eingeben");
console.log(input);
```
...
## Vorzeitiger `return` (Guard Clauses)

Ein `return` muss nicht nur am Ende einer Funktion stehen. Mit einem
**vorzeitigen `return`** kann eine Funktion sofort beendet werden,
z. B. wenn Eingaben ungültig sind oder ein Sonderfall vorliegt.

```js
function divide(a, b) {
  if (b === 0) {
    return undefined; // vorzeitiger Abbruch
  }
...
Das macht Funktionen oft **einfacher lesbar**, weil Sonderfälle früh
behandelt werden und der „normale“ Ablauf danach klarer ist.
...
function testFunction(firstParam, secondParam) {
  ...
  // Rückgabewert - meist am Ende der Funktion
  return x;
}
```
...
Lokale Variablen können im Rumpf der Funktion deklariert werden, sodass dies ihr Gültigkeitsbereich ist,
...
Außerdem können Funktionen **verschachtelt aufgerufen** werden
(Funktionskomposition), z. B. `f(g(h(x)))`:
...
Auch wenn eine Funktionsdeklaration erst später oder am Ende eines Skripts bzw.
...
Das gilt nicht für Funktionsausdrücke oder Arrow-Funktionen; dennoch ist es
in der Regel empfehlenswert, Funktionen am Anfang einer Datei zu deklarieren,
damit der Code übersichtlicher, lesbarer und leichter nachzuvollziehen ist.
</Callout>
...
- Dies wird „_call-by-value_" genannt, weil nur die Werte (bei Objekten: Referenzwerte) und nicht die Variablen selbst übergeben werden.

<Callout type="info">
Bei Objekten als Parameter gilt eine Besonderheit: Änderungen an
Eigenschaften eines Objekt-Parameters wirken sich auch außerhalb
der Funktion aus. Dies wird verständlicher, wenn wir später
