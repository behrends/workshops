...
Für Authentifizierung via SSH-Keys müssen in `~/.ssh/config` zwei Einträge existieren,
einer für `gihub.com` (Standard GitHub-Account) und z. B. `github-dummy` für den zweiten GitHub-Account.

- GitHub-Projekt erstellen (`behrends/repo.git`)
- zweiten GitHub-Account (`dummydhbw`) als _Collaborator_ hinzufügen
- erster GitHub-Account klont das Projekt auf dem Rechner

Als zweiter GitHub-User (`dummydhbw`) das Projekt des ersten Nutzers klonen:

```
git clone git@github-dummy:behrends/repo.git repo-dummy
```
...
```
cd Projektordner
git config user.name "dummydhbw"
git config user.email "dummydhbw@users.noreply.github.com"
```

&rarr; Nun kann der zweite GitHub-Account (`dummydhbw`) lokal Änderungen vornehmen und pushen.
</Callout>
...
„Merge“ ist wohl am gängigsten und am Einfachsten zu verstehen. Dies erzeugt einen neuen Commit
mit den „gemergten“ Änderungen (lokal und remote).

Für einmaliges Pullen mit `merge` kann die passende Option gesetzt werden: 

```bash
git pull --no-rebase
```

Der zweite Benutzer sollte die Merge-Strategie für dieses Projekt festlegen, bevor er pullt.
...
Danach klappt `git pull` und die Änderungen können automatisch gemerged 
werden, weil sie sich nicht überschneiden. 
