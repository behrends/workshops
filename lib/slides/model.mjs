...
function routeSegmentsFromContentPath(contentPath) {
  const normalizedPath = contentPath.split(path.sep).join('/');
  ...
  if (relativePath.endsWith('/index.mdx')) {
    ...
    return parentDir ? parentDir.split('/') : [];
  }

  if (relativePath.endsWith('.mdx')) {
    return relativePath.slice(0, -'.mdx'.length).split('/');
  }
...
export function routeSegmentsFromContentFile(cwd, filePath) {
  ...
  return routeSegmentsFromContentPath(relativePath) || [];
}
...
function resolveSlideLink(url, contentPath) {
  ...
  const routeSegments = routeSegmentsFromContentPath(contentPath);

  if (!routeSegments) {
    return url;
  }

  const currentRoute = '/' + routeSegments.join('/');

  try {
    const resolved = new URL(url, `https://slides.local${currentRoute || '/'}`);
