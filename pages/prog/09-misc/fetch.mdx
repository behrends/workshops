...
<Callout type="info">
Ab Version 18 ist `fetch` in Node.js enthalten (vorerst als experimentelles Feature).
</Callout>

In Node.js bzw. replit.com kann die Funktionalität 
...
Danach kann `fetch` wie folgt zur Verwendung im Code 
importiert werden:
...
[`try…catch`-Blöcken](/prog/09-misc/try-catch) umgeben werden, um 
