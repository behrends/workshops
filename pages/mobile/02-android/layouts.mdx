...
**&xrarr; Für „echte“ Projekte lohnt es sich, das 
...
der standardmäßige Ansatz zur UI-Entwicklung in Android (siehe z.B. [„_Get started with Android_“](https://developer.android.com/get-started/overview) in den Android Developer Docs).
...
- Alles unter der ersten Zeile löschen (`<?xml … >`)
- Dann eingeben `<Linea` und `LinearLayout` aus der Auto-Vervollständigung wählen
- Auf dem rot unterkringeltem Text Tasten `alt`+`enter` drücken und `Add Android Prefix` wählen
- für `layout_width` und `layout_height` den Wert `"match_parent"` eingeben
- Element mit `</LinearLayout>` schließen
- Die restlichen Werte mit Auto-Vervollständigung hinzufügen(`android:orientation`, `xml:tools`, `tools:context`)
</Callout>
...
„Drag-and-Drop“ in das `Linear-Layout` zu ziehen. Eventuell erhalten wir zusätzliche Attribute im XML.
...
[UI-Guide zu Linear Layouts](https://developer.android.com/develop/ui/views/layout/linear) (mit Empfehlung/Verweis auf Compose…)
