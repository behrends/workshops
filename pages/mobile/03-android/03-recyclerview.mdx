...
Hinzufügen im Layout-Editor (Kategorie „_Containers_“) oder direkt im XML-Layout.
Dazu setzen wir auch ein passendes `id`-Attribut mit dem Wert `recyclerView` 
und nutzen den `LinearLayoutManager` als Layout-Manager:
...
definiert werden. In unserer Todo-Liste genügt es, die Elemente mit dem vom Android-SDK 
vordefinierten `LinearLayoutManager` untereinander darzustellen.
...
eine eigene `Todo`-Klasse definiert werden.
...
- `onBindViewHolder`: Bindet die Daten eines Listenelements an eine `View`.
...
kann sie auch wi folgt als Lambda-Ausdruck geschrieben werden:
...
mit dem Wert des Listenelements befüllt:
