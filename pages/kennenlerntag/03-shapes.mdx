import { Callout, Tabs } from 'nextra/components';

# Formen und Farben (25 Minuten)

Wir beginnen nun, mit p5.js das DHBW-Logo zu zeichnen.

![p5.js-Editor](/images/kennenlerntag/dhbw-logo.png)

Dabei gehen wir schrittweise vor, indem wir verschiedene p5.js-Funktionen zur Erstellung von Formen in passenden Farben benutzen. 

## Quadrate (5 Minuten)

Zuerst erstellen wir das rote Quadrat auf der linken Seite des Logos. Dazu können wir die Funktion `square()` benutzen, die drei Parameter hat:

```js
square(x,y,length);
```

Mit `x` und `y` legen wir die Koordination der linken oberen Ecke des Quadrats fest und `width` bestimmt die Seitenlänge des Quadrats.

<Callout type="info" emoji="👨🏻‍💻">
Quadrat mit folgenden Eigenschaften erstellen:

- Position auf der x-Achse etwas links von der Mitte 
- Position auf der y-Achse genau in der Mitte
- Seitenlänge von `120`
</Callout>

![Quadrat](/images/kennenlerntag/square1.png)

<Tabs items={['Lösung:', 'Code']}>
  <Tabs.Tab>Siehe Code</Tabs.Tab>
  <Tabs.Tab>
  ```js
  square(75, 140, 120);
  ```
  </Tabs.Tab>
</Tabs>

## Farben und Ränder (4 Minuten)

Die Funktion `fill()` funktioniert ähnlich wie `background()`. Durch Angabe eines Farbwerts in `fill()` wird die Farbe festgelegt, mit der Formen aufgefüllt werden. Für das DHBW-Logo verwenden wir einen bestimmten Wert für die passende rote Farbe:

```js
fill("#EA1E26");
```

&xrarr; Vor dem Aufruf von `square()` einfügen.

<Callout>
Der Wert `"#EA1E26"` legt bestimmte Wert für den Anteil von rot (`EA`), blau (`1E`) und grün (`26`) fest. Mit `#` wird signalisiert, dass es sich um einen hexadezimalen Wert handelt (Basis 16). <br/> 
&xrarr; Zahlendarstellung werden im 1. Semester behandelt.

Bemerkung: In p5.pjs stehen uns alle Möglichkeiten der Farbbestimmung aus CSS zur Verfügung.
</Callout>

Nun haben wir noch einen schwarzen Rand um das Quadrat herum. Durch den Aufruf von 

```js
noStroke();
```

vor `square` deaktivieren wir die Ränder der dargestellten Formen.

## Quadrate mit „runden“ Ecken (1 Minute)

Ein vierter Parameter im Aufruf von `square()` legt die Abrundung der Ecken fest:

```js
square(75, 140, 120, 4);
```

Somit wurde bereits das rote Quadrat des Logos erzeugt.

## Graues Quadrat (5 Minuten)

Nun soll rechts vom roten Quadrat ein entsprechendes graues Quadrat dargestellt werden.

<Callout type="info" emoji="👨🏻‍💻">
Quadrat mit folgenden Eigenschaften erstellen:

- Position auf der x-Achse etwas rechts von der Mitte 
- Position auf der y-Achse genau in der Mitte
- Seitenlänge von `120`
- Wert des Grautons: `#73818D`
</Callout>

![Zwei Quadrate nebeneinander](/images/kennenlerntag/square2.png)

<Tabs items={['Lösung:', 'Code']}>
  <Tabs.Tab>Siehe Code</Tabs.Tab>
  <Tabs.Tab>
  ```js
  fill("#73818D");
  square(205, 140, 120, 4);
  ```
  </Tabs.Tab>
</Tabs>

## Dreiecke (5 Minuten)

Als Zwischenschritt für die Weiterenwicklung des DHBW-Logos sollen zwei kleinere Dreiecke an den Quadraten gezeichnet werden.

Zunächst ein graues Dreieck unterhalb des roten Quadrats:

```js
fill("#73818D");
triangle(135, 260, 195, 260, 135, 320);
```

`triangle()` erwartet 6 Parameter:

```js
triangle(x1, y1, x2, y2, x3, y3);
```

`(x1,y1)`, `(x2,y2)` und `(x3,y3)` legen die Koordinaten der drei Ecken des Dreiecks fest.

<Callout type="info" emoji="👨🏻‍💻">
Dreieck mit folgenden Eigenschaften erstellen:

- Farbe: `#EA1E26` (rot)
- Position: oberhalb des grauen Quadrats
- Größe wie das graue Dreieck unter dem roten Quadrats(s.o.)
- Siehe folgende Abbildung für das gewünschte Aussehen
</Callout>


![Quadrate mit Dreicken](/images/kennenlerntag/square-triangles.png)

<Tabs items={['Lösung:', 'Code']}>
  <Tabs.Tab>Siehe Code</Tabs.Tab>
  <Tabs.Tab>
  ```js
  fill("#EA1E26");
  triangle(205,140, 265, 80, 265, 140);
  ```
  </Tabs.Tab>
</Tabs>

## Vierecke (10 Minuten)

Durch ein Viereck innerhalb des roten Quadrats soll der nächste Teil des DHBW-Logos wie folgt erscheinen:

![Viereck im roten Quadrat](/images/kennenlerntag/square-quad.png)

Die Farbe ist ein dunkles Rot mit dem Wert `"#86171C"`.

Wir zeichnen ein Viereck (engl. „_quadrangle_“) mit der p5.js-Funktion `quad()`, die vier Koordinaten und daher acht Parameter benötigt (beim Dreieck waren es drei Koordinaten bzw. sechs Parameter):

```js
fill("#86171C");
quad(195, 145, 195, 260, 135, 260, 135, 200);
// Die ersten Koordinaten des Vierecks stehen hier für die Ecke
// rechts oben“ und die folgenden verlaufen im Uhrzeigersinn
```

<Callout>
Tipp: Manchmal hilft es, die sich an den Koordinaten bereits
vorhandener Formen zu orientieren. Bei uns ist dies das rote
Quadrat. Dadurch wird der Programmcode leichter nachvollziehbar:

```js
// Die ersten Koordinaten des Vierecks stehen hier für die Ecke
// rechts oben“ und die folgenden verlaufen im Uhrzeigersinn
quad(
    75 + 120, // x1 rotes Quadrat + Länge (Quadrat)
    140 + 5, // y1 rotes Quadrat + kleiner Abstand
    75 + 120, // x1 rotes Quadrat + Länge (Quadrat)
    140 + 120, // y1 rotes Quadrat + Länge (Quadrat)
    75 + 120 / 2, // x1 rotes Quad. + halbe Länge
    140 + 120, // y1 rotes Quad. + Länge
    75 + 120 / 2, // x1 rotes Quad. + halbe Länge
    140 + 120 / 2 // y1 rotes Quad. + halbe Länge
  );
```

Die Werte oben entsprechen dem vorigen Aufruf von `quad()`.

Ausblick: Mit Variablen bzw. Konstanten können bestimmte Werte leichter 
wiederverwendet und für ganze Bereiche des Codes angepasst werden:

```js
const canvasWidth = 600;
const canvasHeight = 600;
const squareSize = 180;
const squareGap = squareSize / 30;
const redSquareStartX = canvasWidth / 2 - squareSize - squareGap;
const redSquareStartY = canvasHeight / 2 - squareSize / 2;
// usw. 
```
</Callout>

<Callout type="info" emoji="👨🏻‍💻">
Versuchen Sie, im grauen Quadrat ein Viereck zu zeichnen, das wie in der folgenden Abbildung aussieht (gleiche Farbe wie das Viereck im roten Quadrat).
</Callout>

![Vierecke in beiden Quadraten](/images/kennenlerntag/square-quad2.png)

<Tabs items={['Lösung:', 'Code']}>
  <Tabs.Tab>Siehe Code</Tabs.Tab>
  <Tabs.Tab>
  ```js
  fill("#86171C");
  quad(
    205,
    140,
    205 + 120 / 2,
    140,
    205 + 120 / 2,
    140 + 120 / 2,
    205,
    140 + 120 - 5
  );
  ```
  </Tabs.Tab>
</Tabs>

Nun haben wir fast das DHBW-Logo erstellt. Was fehlt noch?

<Tabs items={['Antwort:', '👇']}>
  <Tabs.Tab>Siehe 👉 </Tabs.Tab>
  <Tabs.Tab>
Wir sollten das Hilfsgitter entfernen.

Wenn das DHBW-Logo groß genug ist, dann ist zu erkennen, dass die Dreiecke eigentlich auch Vierecke sind…
  </Tabs.Tab>
</Tabs>

## Dreiecke durch Vierecke ersetzen (Optional)

Wer schnell ist bzw. wenn es genügend Zeit gibt, dann besteht eine gute Übung darin, den Code so anzupassen, dass die Hilfslinien für die Koordinaten mit Kommentaren ausgeblendet werden und die Dreiecke auch Vierecke sind. Unser Entwurf sollte folgender Abbildung gleicht:

![Dreiecke als Vierecke](/images/kennenlerntag/square-quad3.png)

<Callout>
Dazu muss bestehender Code verändert bzw. angepasst werden. Dies kommt im Alltag der Programmierung viel häufiger vor als das Schreiben neuen Codes.
</Callout>

<Callout type="warning">
Im echten DHBW-Logo sind auch die Ecken der Vierecke abgerundet…
</Callout>

<Tabs items={['Kompletter Code:', '👇']}>
  <Tabs.Tab>👉</Tabs.Tab>
  <Tabs.Tab>
  ```js
function setup() {
  createCanvas(400, 400);
}

function draw() {
  background("white");
  // drawGrid();

  noStroke();
  fill("#EA1E26");
  square(75, 140, 120, 4);

  fill("#73818D");
  square(205, 140, 120, 4);

  fill("#73818D");
  quad(
    75 + 120 / 2,
    140 + 120,
    75 + 120,
    140 + 120,
    75 + 120,
    140 + 120 + 5,
    75 + 120 / 2,
    140 + 120 + 120 / 2
  );


  fill("#EA1E26");
  quad(
    205,
    140,
    205,
    140 - 5,
    205 + 120 / 2,
    140 - 120 / 2 - 5,
    205 + 120 / 2,
    140
  );

  fill("#86171C");
  quad(
    75 + 120, // x1 rotes Quadrat + Länge des Quadrats
    140 + 5, // y1 rotes Quadrat + kleiner Abstand
    75 + 120, // x1 rotes Quadrat + Länge des Quadrats
    140 + 120, // y1 rotes Quadrat + Länge des Quadrats
    75 + 120 / 2, // x1 rotes Quadrat + halbe Länge des Quadrats
    140 + 120, // y1 rotes Quadrat + Länge des Quadrats
    75 + 120 / 2, // x1 rotes Quadrat + halbe Länge des Quadrats
    140 + 120 / 2 // y1 rotes Quadrat + halbe Länge des Quadrats
  );

  fill("#86171C");
  quad(
    205,
    140,
    205 + 120 / 2,
    140,
    205 + 120 / 2,
    140 + 120 / 2,
    205,
    140 + 120 - 5
  );
}

function drawGrid() {
  stroke("grey");
  for (let i = 1; i <= 4; i++) {
    line(0, (400 / 4) * i, 400, (400 / 4) * i); // horizontal line
    line((400 / 4) * i, 0, (400 / 4) * i, 400); // vertical line
  }
}
  ```
  </Tabs.Tab>
</Tabs>

[DHBW Logo mit Konstanten](https://editor.p5js.org/behrends/sketches/RIYuNYMDk)