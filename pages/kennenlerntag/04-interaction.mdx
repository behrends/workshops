...
Im jedem Durchlauf von `draw()` können wir auf `mouseX` und `mouseY` zugreigen, um die aktuelle x- bzw. y-Koordinate des Mauszeigers zuzugreifen.

Das folgende Beispiel zeigt, wie die Farbe des grauen Quadrats auf einen blaugrünen Ton (_cyan_) wechselt, wenn der Mauszeiger in der Fläche des grauen Quadrats ist:
